...
def cat():
    global cat_position, last_time, cat_speed
    current_time = pygame.time.get_ticks()
    delta_time = (current_time - last_time) / 1000.0  
    if cat_move:
        
        cat_position[2] -= cat_speed * delta_time 
    glPushMatrix()
    glScalef(*scene_scale)
    glTranslatef(*scene_trans)
    glTranslatef(cat_position[0], cat_position[1], cat_position[2])
    glRotatef(1, 0, 1, 0)  
    glColor3f(1, 0.5, 0)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_a:
                    glRotatef(5, 0, -1, 0)  # Rotate around X-axis positive
                if event.key == pygame.K_d:
                    glRotatef(5, 0, 1, 0)  # Rotate around X-axis negative
                if event.key == pygame.K_w:
                    glRotatef(5, -1, 0, 0)  # Rotate around Y-axis positive
                if event.key == pygame.K_s:
                    glRotatef(5, 1, 0, 0)  # Rotate around Y-axis negative
